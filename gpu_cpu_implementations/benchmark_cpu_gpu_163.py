# -*- coding: utf-8 -*-
"""
Benchmark CPU vs GPU scalar multiplication for GF(2^163).
...
CPU_SCRIPT = os.path.join(DIR, "scalar_mult_163.py")
GPU_SCRIPT = os.path.join(DIR, "GPU_kernal_ecc_scalar_163.py")
NUM_RUNS = 10

# --- Run CPU benchmark ---
cpu_times = []
print("=" * 50)
print("Running CPU benchmark (GF(2^163))")
print("=" * 50)
...
gpu_run_avgs = []
print("\n" + "=" * 50)
print("Running GPU benchmark (GF(2^163))")
print("=" * 50)
for run in range(1, NUM_RUNS + 1):
    result = subprocess.run(["python3", GPU_SCRIPT], capture_output=True, text=True)
    times = [float(m) for m in re.findall(r"GPU scalar multiplication time:\s+([\d.]+)\s+ms", result.stdout)]
...
lines = []
lines.append("=" * 65)
lines.append("CPU vs GPU Benchmark - GF(2^163) Scalar Multiplication")
lines.append("GPU: NVIDIA Jetson Orin Nano Super")
lines.append("=" * 65)
...
stats_path = os.path.join(DIR, "benchmark_cpu_gpu_163_jetson_statistics.txt")
...
axes[0].set_title("GF(2^163) Scalar Multiplication\nCPU vs GPU (Jetson Orin Nano Super) Time per Run", fontsize=13)
...
axes[1].set_title("GF(2^163) Scalar Multiplication\nCPU vs GPU (Jetson Orin Nano Super) Comparison", fontsize=13)
axes[1].set_xticks(x)
...
plt.tight_layout()
plot_path = os.path.join(DIR, "benchmark_cpu_gpu_163_jetson_plot.png")
