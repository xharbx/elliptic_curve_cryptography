# -*- coding: utf-8 -*-
"""
Full GPU Benchmark: GF(2^571) Scalar Multiplication on Jetson Orin Nano Super
...
# ============================================================
# GPU Kernel Code (from GPU_kernal_ecc_scalar_571.py, silent version)
# ============================================================
kernel_code = r'''
...
// ============================================================
// Itoh-Tsujii inversion for GF(2^571)
// ============================================================
...
// ============================================================
// Lopez-Dahab Point Double for GF(2^571) - silent version
// ============================================================
...
// ============================================================
// Lopez-Dahab Point Add (Z1 = 1) for GF(2^571) - silent version
// ============================================================
...
// ============================================================
// Scalar multiplication (Lopez-Dahab double & add) GF(2^571) - silent version
// ============================================================
...
# ============================================================
print("=" * 70)
print("Full GPU Benchmark: GF(2^571) Scalar Multiplication")
...
Xout = cp.zeros(9, dtype=cp.uint64)
Yout = cp.zeros(9, dtype=cp.uint64)
Zout = cp.zeros(9, dtype=cp.uint64)
...
for w in range(5):
    ...
    k_limbs = [(k_int >> (64*i)) & ((1 << 64) - 1) for i in range(9)]
...
# ============================================================
report_path = os.path.join(os.getcwd(), "full_benchmark_571.txt")

with open(report_path, "w") as f:
    f.write("=" * 75 + "\n")
    f.write("FULL GPU BENCHMARK REPORT\n")
    f.write("GF(2^571) Scalar Multiplication - sect571r1\n")
    ...
    f.write("-" * 75 + "\n")
    f.write(f"  Kernel Name:               scalar_mult_571\n")
    ...
    f.write(f"  Input Buffers:             9 x 9 x uint64 = {9 * 9 * 8} bytes (Px,Py,Pz,a,b,k,Xout,Yout,Zout)\n")
    ...
    # Per-run table
    hex_width = (M + 3) // 4  # 143 chars for 571-bit values
    f.write("  Per-Run Latency Detail:\n")
    f.write(f"  {'Run':<6}{'Wall (ms)':<14}{'CUDA (ms)':<14}{'Scalar k (hex)':<150}{'x_aff (hex)'}\n")
    f.write("  " + "-" * 73 + "\n")
    for run, k_val, x_val, w_ms, c_ms in results_log:
        f.write(f"  {run:<6}{w_ms:<14.4f}{c_ms:<14.4f}0x{k_val:0{hex_width}x}   0x{x_val:0{hex_width}x}\n")
    f.write("\n")
    ...
    f.write("SUMMARY\n")
    f.write("=" * 75 + "\n")
    f.write(f"  Curve:                     sect571r1 (GF(2^571))\n")
    f.write(f"  Algorithm:                 Lopez-Dahab double-and-add\n")
    f.write(f"  Inversion:                 Itoh-Tsujii (addition chain)\n")
    f.write(f"  Field Representation:      9 x 64-bit limbs (polynomial basis)\n")
    f.write(f"  GPU:                       {gpu_name}\n")
