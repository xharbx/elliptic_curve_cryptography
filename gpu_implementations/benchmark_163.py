# -*- coding: utf-8 -*-
"""
Benchmark GPU_kernal_ecc_scalar_163.py over 10 runs.
...
SCRIPT = os.path.join(os.path.dirname(__file__), "GPU_kernal_ecc_scalar_163.py")
NUM_RUNS = 10

all_run_times = []  # list of lists: per-test times for each run
run_averages = []
...
for run in range(1, NUM_RUNS + 1):
    ...
    times = [float(m) for m in re.findall(r"GPU scalar multiplication time:\s+([\d.]+)\s+ms", output)]
...
# Flatten all individual test times across all runs
all_times = [t for run_times in all_run_times for t in run_times]

# --- Statistics ---
stats_lines = []
stats_lines.append("=" * 60)
stats_lines.append("GPU GF(2^163) Scalar Multiplication - Benchmark Statistics")
stats_lines.append("=" * 60)
...
stats_path = os.path.join(out_dir, "benchmark_163_statistics.txt")
...
axes[0].set_title("GF(2^163) GPU Scalar Multiplication\nAverage Time per Run", fontsize=13)
axes[0].set_xticks(runs)
axes[0].legend(fontsize=11)
axes[0].grid(True, alpha=0.3)

# Right: Box plot of per-test times for each run
axes[1].boxplot(all_run_times, labels=[str(i) for i in runs])
axes[1].set_xlabel("Run", fontsize=12)
axes[1].set_ylabel("Time (ms)", fontsize=12)
axes[1].set_title("GF(2^163) GPU Scalar Multiplication\nPer-Test Time Distribution by Run", fontsize=13)
axes[1].grid(True, alpha=0.3)

plt.tight_layout()
plot_path = os.path.join(out_dir, "benchmark_163_plot.png")
