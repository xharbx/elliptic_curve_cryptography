...
for t in range(num_tests):
    ...
    print(f"Test #{t+1:02d} | GPU time: {gpu_time:.2f} ms | x_aff = 0x{Xr:059x}")
...
print("\n" + "=" * 60)
print("GPU Scalar Multiplication Performance - GF(2^233)")
print("=" * 60)
...
# ============================================================
stats_file = "gpu_233_performance_stats.txt"
with open(stats_file, "w") as f:
    f.write("=" * 70 + "\n")
    f.write("GPU Scalar Multiplication Performance - GF(2^233) sect233r1\n")
    ...
    f.write("-" * 70 + "\n")
    f.write(f"{'Run':<6}{'Time (ms)':<14}{'Scalar k (hex)':<62}{'x_aff (hex)'}\n")
    f.write("-" * 70 + "\n")
    for run, k_val, x_val, y_val, t_ms in results_log:
        f.write(f"{run:<6}{t_ms:<14.4f}0x{k_val:059x}   0x{x_val:059x}\n")
...
plt.title('GPU Scalar Multiplication Performance\nGF(2^233) - sect233r1 - Lopez-Dahab (50 Runs)', fontsize=14, fontweight='bold')
plt.grid(True, alpha=0.3)
plt.legend(fontsize=10, loc='upper right')
plt.xticks(range(1, num_tests + 1, 2))
plt.tight_layout()

plot_file = 'gpu_233_performance_plot.png'
